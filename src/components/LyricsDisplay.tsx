...
interface LyricsDisplayProps {
  ...
  bpm: string;
  setLyrics: (updater: React.SetStateAction<SongSection[]>) => void;
  isLoading: boolean;
  error: string | null;
...
export const LyricsDisplay: React.FC<LyricsDisplayProps> = React.memo(({
  ...
  return (
    <>
      <div id="lyrics-display-section" ref={displayRef} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700 shadow-lg h-full flex flex-col">
        <div 
            className="flex-grow p-4 flex flex-col relative overflow-hidden"
        >
          {title && <h2 className="text-2xl font-bold text-purple-300 mb-4">{title}</h2>}
